...
- **即时取消**：可立即取消迁移任务，即使在大文件上传过程中也能立即中断
- **错误处理**：可配置的错误处理和重试逻辑
- **持久化状态**：任务在容器重启后仍然保留
...
4. Worker 协程处理任务：
   ...
   - 递归扫描源文件夹
   - 在 ZimaOS 上创建目录结构
   - 使用基于 context 的可取消机制和分块重试逻辑上传文件
   - 实时更新进度
   - 可通过 context 取消机制立即中断正在进行的文件上传
5. **文件校验阶段**（如果启用）：
   - 遍历所有已上传的文件
...
- **基于 Context 的取消**：使用 Go context 立即取消正在进行的文件上传
- **任务持久化**：所有任务存储在 SQLite 中，重启后可恢复
- **分块上传**：大文件以 10MB 块上传，支持可取消的 reader
- **指数退避**：失败的上传会以指数延迟重试
- **进度跟踪**：实时进度和速度计算
